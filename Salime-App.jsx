...
function isOpenAt(place, minute) {
  ...
  /*
   * Lugares reales traen su propio horario en
   * place.hours ([apertura, cierre]) cuando Geoapify lo informa.
   * Si no lo trae, se asume abierto.
   */
...
/* ------------------------------------------------------------------ */
/* Lugares reales (OpenStreetMap + Overpass vía api/lugares.js)        */
/*                                                                      */
/* Con ubicación, los pools mock NO son fallback. Solo se muestran     */
/* lugares devueltos por la fuente real.                                */
/* ------------------------------------------------------------------ */
...
const POOL_KEYS = new Map([
  // Estas son intenciones que api/lugares.js entiende directamente.
  // Antes se enviaban nombres internos como "CENA" o "FINAL", que
  // terminaban cayendo en la intención general y podían traer parques
  // u otros POIs que no correspondían a "comer".
  [CENA, "comer"],
  [BEBIDA, "beber"],
  [FINAL, "paseo"],
  [CULTURA, "cultura"],
 ...
 * ciudad/zona. Nunca lanza: si algo falla, devuelve [].
 */
...
function pick(
  ...
  // No reutilizamos un lugar ya elegido. Si el pool no tiene otra
  // opción real, dejamos el paso vacío en vez de repetirlo.
  if (candidates.length === 0) {
    return null;
  }
...
function getSteps({
  ...
  // "De día" es una franja genérica: no debe caer en "tarde".
  // Primero respetamos la intención de día completo/mediodía y recién
  // después una tarde explícita.
...
async function generatePlan({
   ...
   * Con ubicación, si falla la consulta o no hay lugares reales para
   * alguna categoría, esa categoría queda vacía. No se usan mocks.
   */
  ...
  return steps.map((step) => {
    ...
    const activePool = location
      ? (poolKey && poolOverrides[poolKey] ? poolOverrides[poolKey] : [])
      : step.pool;
...
function buildVenueWhy(venue, { budgetLabel, moodLabel, close, outdoor, hasKids, daytimeGeneric, morning, afternoon, night } = {}) {
  if (venue?.why) return venue.why;

  const reasons = [];

  if (hasKids) reasons.push("es una opción apta para ir en familia");
  if (outdoor && venue?.outdoor) reasons.push("tiene una propuesta al aire libre");
  if (close && typeof venue?.dist === "number") reasons.push("queda entre las opciones más cercanas");
  if (moodLabel && Array.isArray(venue?.mood) && venue.mood.length) {
    const wanted = String(moodLabel).toLowerCase();
    if (venue.mood.some((m) => String(m).toLowerCase() === wanted)) {
      reasons.push(`encaja con el clima ${moodLabel.toLowerCase()}`);
    }
  }
  if (daytimeGeneric) reasons.push("funciona para un plan de mediodía y tarde temprana");
  else if (morning) reasons.push("funciona para la mañana");
  else if (afternoon) reasons.push("funciona para la tarde");
  else if (night) reasons.push("funciona para la noche");
  if (budgetLabel) reasons.push(`está dentro de la opción ${String(budgetLabel).toLowerCase()}`);

  return reasons.length
    ? reasons.slice(0, 2).join(" y ")
    : "es una opción real encontrada para tu zona y compatible con el plan";
}
            ...
            <div className="tl-content">
              ...
              <div className="tl-card">
                ...
                <p className="tl-card-why">
                  Porque{" "}
                  {buildVenueWhy(step.venue, guideContext)}.
                </p>
              </div>
            </div>
          </div>
